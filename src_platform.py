...
from platforms.vmware_vsphere.interface import VsphereInterface
...
class Platform(Base):
    def __init__(self, platform_id, context=None):
...
class VMWarePlatform(Platform):
    ...
    def __init__(self, platform_id, context=None):
        super(VMWarePlatform, self).__init__(platform_id, context)
    ...
    def config(self):
        if not self._config:
            self._config = self.context.v2v_pg.query_src_platform(self.platform_id)
        return self._config
