...
try:
    ...
    # 模拟image_tool模块，以便测试能够正常运行
    def get_nbd_device(img_path):
        return "/dev/nbd0"
    
    def open_nbd(dev_path, img_path, readonly=1):
        pass
    
    def clean_mnt(mnt_dir):
        return 0
    
    def close_nbd(dev_path):
        pass
from contextlib import contextmanager
from utils.cli.exec import normal_exec
try:
    from utils.pitrix_folder import PitrixFolder
except ImportError:
    # 模拟PitrixFolder类，以便测试能够正常运行
    class PitrixFolder:
        @staticmethod
        def get_image_mnt(img_path):
            import os
            return os.path.join("/tmp", "image_mnt", os.path.basename(img_path))
