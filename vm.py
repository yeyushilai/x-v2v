...
from migration.migration_coordinator import (
...
class VMSession(VMSessionBase):
    ...
    def info(self):
        if not self._info:
            self._info = self.context.v2v_pg.query_vm(self.session_id)
        return self._info
    ...
    @property
    def task(self):
        return MigrateTask(self.task_id, self.context)
    ...
    def migrate(self):
        ...
        with self._migrate() as pre_check:
            ...
            # 失败后清理垃圾
            if self.context.zone_pg.query_image(self.dst_vm_image_id):
                columns = dict(status=ImageStatus.CEASED.value)
                logger.info("vm has migrate failed, try to ceased "
                            "image, session id: {session_id}"
                            "".format(session_id=self.session_id))
                self.context.zone_pg.update_image(self.dst_vm_image_id, columns=columns)
    ...
    def update_to_pg(self, data=None):
        """更新虚拟机任务信息到数据库"""
        self.context.v2v_pg.update_vm(session_id=self.session_id, columns=data)
